...
def test_data_service_creates_data():
    ...
    data_out = data_service.create(data_in)
...
def test_data_service_reads_data_by_id():
    ...
    data_service = DataService()
    data_out = data_service.read_by_id(data_id)
    mock_db_teardown()
...
def test_data_service_updates_data():
    ...
    data_service = DataService()
    data_out = data_service.update(data_in)
    mock_db_teardown()
...
def test_read_latest_qa_model_by_type():
    ...
    db.session.add(data_in)
    db.session.commit()
    response = tester.get('/' + QA_MODEL_CONTROLLER_VERSION + '/models/BERT/latest',
                          content_type='application/json')
